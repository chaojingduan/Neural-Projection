...
def get_and_init_FC_layer(din, dout):
    print("get_and_init_FC_layer start")
    li = nn.Linear(din, dout)
    #init weights/bias
    nn.init.xavier_uniform_(li.weight.data, gain=nn.init.calculate_gain('relu'))
    li.bias.data.fill_(0.)
    print("get_and_init_FC_layer finish")
    return li


def get_MLP_layers_true(dims, doLastRelu):
    print("get_MLP_layers_true start")
    layers = []
    ...
    print("get_MLP_layers_true finish")
    return layers


def get_MLP_layers(dims, doLastRelu):# test
    print("get_MLP_layers start")
    layers = []
    for i in range(1, len(dims)):
        layers.append(get_and_init_FC_layer(dims[i-1], dims[i]))
        if i==len(dims)-1 and not doLastRelu:
...
class PointNetGlobalMax1(nn.Sequential):
    '''BxNxdims[0] -> Bxdims[-1]'''

    def __init__(self, dims, doLastRelu=False):
        print("PointNetGlobalMax1 start")
        layers = [
            PointwiseMLP(dims, doLastRelu=doLastRelu),      #BxNxK
            #GlobalPool(nn.AdaptiveMaxPool2d((1, dims[-1]))),#BxK
        ]
        super(PointNetGlobalMax1, self).__init__(*layers)
        print("PointNetGlobalMax1 finish")

class PointNetGlobalMax(nn.Sequential):
    '''BxNxdims[0] -> Bxdims[-1]'''

    def __init__(self, dims, doLastRelu=False):
        print("PointNetGlobalMax start")
        layers = [
            PointwiseMLP(dims, doLastRelu=doLastRelu),      #BxNxK
            GlobalPool(nn.AdaptiveMaxPool2d((1, dims[-1]))),#BxK
        ]
        super(PointNetGlobalMax, self).__init__(*layers)
        print("PointNetGlobalMax finish")
...
class PointNetVanilla1(nn.Sequential):

    def __init__(self, MLP_dims, MLP_doLastRelu=False):
    #def __init__(self, MLP_dims, FC_dims, MLP_doLastRelu=False):
        print("PointNetVanilla1 start")
        #assert(MLP_dims[-1]==FC_dims[0])
        layers = [
            PointNetGlobalMax1(MLP_dims, doLastRelu=MLP_doLastRelu),#BxK
        ]
        print("PointNetVanilla layers" , layers)
        #layers.extend(get_MLP_layers(FC_dims, False))
        #print("PointNetVanilla layers after extend" , layers)
        print("PointNetVanilla1 finish")
        super(PointNetVanilla1, self).__init__(*layers)

class PointNetVanilla(nn.Sequential):

    def __init__(self, MLP_dims, MLP_doLastRelu=False):
    #def __init__(self, MLP_dims, FC_dims, MLP_doLastRelu=False):
        print("PointNetVanilla start")
        #assert(MLP_dims[-1]==FC_dims[0])
        layers = [
