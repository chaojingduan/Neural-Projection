...
class PointNetfeat(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.global_feat:
            ...
            return torch.cat([x, localfeat], 1), trans, trans_feat
...
class PointNetCls(nn.Module):
    ...
    def forward(self, x):
        x, trans, trans_feat = self.feat(x) # trans: (b, 3, 3) trans_feat: (b, 64, 64), x: (b, 1024)
...
class PointNetDenseCls(nn.Module):
    def __init__(self, k = 2, feature_transform=False):
        ...
        self.conv1 = torch.nn.Conv1d(640, 256, 1)
